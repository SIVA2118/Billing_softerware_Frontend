...
const generateBillingPdf = async (isoDate, invoices) => {
  ...
  const pdfInvoices = [...invoices].reverse();
  const totalPages = Math.max(1, Math.ceil(pdfInvoices.length / 3));
  let pageNumber = 1;
  let y = margin + 60;

  drawHeader(pageNumber, totalPages);

  pdfInvoices.forEach((inv, idx) => {
    ...
    if (idx < pdfInvoices.length - 1 && y + 80 > pageHeight - margin - pageFooterHeight) {
      doc.addPage();
      pageNumber += 1;
      y = margin + 60;
      drawHeader(pageNumber, totalPages);
...
const BillingCalendar = () => {
  ...
  return (
    <div className="page-shell" style={S.container} id="billing-calendar-page">
      <div style={S.header}>
        <div>
          <div style={S.eyebrow}>Operations / Daily Ledger</div>
          <h1 className="page-title-display" style={S.title}>Billing Calendar</h1>
          <p style={S.subtitle}>Review every invoice issued on a selected day.</p>
        </div>
        <div style={S.dateBadge}>
          <span style={S.dateBadgeLabel}>Selected date</span>
          <strong>{selectedDate ? formatLocalDate(selectedDate) : 'Choose a date'}</strong>
        </div>
      </div>
          ...
          <div style={S.card}>
            ...
            <div style={S.cardInner}>
              <div style={S.field} id="calendar-input">
                <div style={S.sectionKicker}>Daily view</div>
          ...
          <div style={S.card}>
            ...
            <div style={S.detailsCardInner}>
              <div style={S.detailsHeader} className="billing-details-header">
                <div>
                  <div style={S.sectionKicker}>Invoice register</div>
...
const S = {
  ...
  eyebrow: {
    marginBottom: '6px', color: 'var(--gold)', fontSize: '0.62rem',
  ...
  dateBadge: {
    ...
    padding: '10px 14px', minWidth: '142px',
    background: 'var(--gold-dim)', border: '1px solid var(--gold-border)', borderRadius: '10px',
    color: 'var(--text-primary)', fontSize: '0.78rem',
  },
  dateBadgeLabel: { color: 'var(--text-muted)', fontSize: '0.62rem', textTransform: 'uppercase', letterSpacing: '1px' },
  card: {
